...
export default function WorkLocationsOnMapForManager() {
  const { data, isLoading } = useWorkLocations({ is_active: 1 });
  if (isLoading) {
    return (
      <div
        className={`shadow-md rounded-xl p-5 h-[30rem] screen1250:h-[22rem] w-full`}
      >
  ...
  return (
    <div
      className={`shadow-md rounded-xl h-[600px] screen1250:h-[600px] w-full overflow-hidden border border-primary-content relative`}
    >
